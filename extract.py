...
from ops.models_extract import TSN
...
def validate(val_loader, model, criterion, epoch, log=None, tf_writer=None):
	...
	#			continue
			...
			#print("-"*50)
			#input("enter any key")
			while flag < len(sift)-1:
				#print(sift)
				#input("enter any key")
				if sift[flag+1] - sift[flag] <= 50:
					flag +=1
				else:
					sift.insert(flag+1, sift[flag]+50)
					flag +=1
			sift.pop()
			print(sift)
			print(URL)
			...
			for i, s in enumerate(sift):
				...
				if len(totalOutput) == 0:
					totalOutput = output.cpu().numpy()
				else:
					totalOutput = np.concatenate((totalOutput,output.cpu().numpy()),axis=0)
					#totalOutput.append(output.cpu().numpy())
				#print("totalOutput size()",totalOutput.shape)
				...
				#oss = criterion(output.squeeze(), target.squeeze())
				#print("loss = ",loss)
			...
			#print(str(URL[0]))
			#print(args.save_scores)
			#print(totalOutput.shape)
			#print(args.save_scores + str(URL[0]))
			if not os.path.isfile(args.save_scores + URL[0]+".npy"):
				np.save(args.save_scores + URL[0], totalOutput)
			#exit()
