...
def word2tensor(folderPath:str=os.path.join(r'/home/share/YouCook',"captions_v2")):
    ...
    engString = str("")
    with open(os.path.join(r'/home/share/YouCook',"annotations","youcookii_annotations_trainval.txt"),"r") as _anntxt:
        text = _anntxt.read()
        ...
        for key, value in database.items():
            ...
            for segments in value["annotations"]:
                engString += segments["sentence"].strip() + str(" ")
        engString = engString.strip()
        eng.addSentence(engString)
    #print(eng.getdict)
    a = eng.getdict()
...
class YouCookDataSetRcg(data.Dataset):
    ...
    def __init__(self, root_path = r'/home/share/YouCook',list_file=r"/home/share/YouCook",
                ...
                transforms=torchvision.transforms.Compose([
                    ...
                    inputsize = 224, hasWordIndex = False, hasPreprocess = False,clipnums:str='',
                ):
    ...
    def _getAnnotation(self):#only include train(1333) and val(457) = 1790
                            #set llist = [key, recipe_type, segment, setence, duration]
        if self.clipnums == '':  #train all
            print("no clip nums")
            #exit()
    ...
    def __getitem__(self, index):   #for dataset iter need
                                    #return [URL, id, annotaion, clip]
                                    #annotation = "cut the fish"
                                    #clip = [[video, caption], [], ...]
        #print("_gettiem")
        #print(self._getMode())
        #print(self.mode)
        ...
        if not self.hasPreprocess:
            ...
            print("annotation")
            print(annotation)
            ann_tensor = self._word2tensor(annotation)
            print(ann_tensor)
            ...
            try: #download successful
                ...
                for idx in range(segment[0], segment[1]):
                    ...
                    if isinstance(video, bool):
                        print("isinstance")
                        return False
                    captions, _ = self._getCaptions(id=id, URL=URL, idx=idx)
                    clip[idx- segment[0]] = [video, captions]
...
if __name__ == "__main__":
    ...
    dataset1 = YouCookDataSetRcg(train=True,inputsize=crop_size, hasWordIndex=True, hasPreprocess=False,clipnums=str(500))
