...
from archs.Transformer import TransformerModel
...
def train(train_loader, model, decoder, criterion, optimizer, epoch, log, tf_writer, index2wordDict):
	...
	#		continue
		...
		decoder_outputs = []
		src_mask = decoder.generate_square_subsequent_mask(target.size(1)).cuda()
		...
		loss = criterion(decoder_output.transpose(0,1).squeeze(), target.squeeze())
